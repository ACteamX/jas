...
//declaring pins for obstacle detection
const int R = 43;
const int G = 44;
...
void loop() {
  ...
  move('F');
  while (!checkObstacle() && !checkCorner()) {
    updateDistance();
...
//Checking Functions
bool checkCorner() {
  if (!checkObstacle() && (distanceFront <= corner_distance))
    return true;
  else return false;
}
...
void cornerTurn() {
  ...
  while (true) {
    updateDistance();
    if (distanceFront >= 26 || checkObstacle())
      break;
  }
  move('S');
  delay(40);
